 ...
 *                                                                                  *
 ************************************************************************************/
function determineOrigin(fileName) {
 ...
 *                                                                                                                      *
 ************************************************************************************************************************/
async function decodeFile(file, gameOrigin) {
 ...
 *                                                                                  *
 ************************************************************************************/
async function partitionCollection(bufferView) {
...
function updateAudioOptions() {
    ...
    for (let i = 0; i < Object.keys(audioCollection[currentGame]).length; ++i) {
        RADIO_SELECT_ELEMENT.children[i].value = Object.keys(audioCollection[currentGame])[i];
        RADIO_SELECT_ELEMENT.children[i].innerText = audioCollection[currentGame][Object.keys(audioCollection[currentGame])[i]][0];
        ...
        if (availableStations.includes(Object.keys(audioCollection[currentGame])[i])) {
            RADIO_SELECT_ELEMENT.children[i].removeAttribute("disabled");
            if (Object.keys(audioCollection[currentGame])[i] === currentStation) {
                RADIO_SELECT_ELEMENT.children[i].setAttribute("selected", "true");
            }
        }
        else {
            RADIO_SELECT_ELEMENT.children[i].setAttribute("disabled", "true");
        }
        console.log(Object.keys(audioCollection[currentGame])[i]);
        console.log(audioCollection[currentGame][Object.keys(audioCollection[currentGame])[i]][0]);
...
async function retrieveJSON() {
    ...
    for (let i = 0; i < JSON_FILES.length; ++i) {
        fetch(JSON_FILES[i]).then(response => {
...
function getRandomTrack(station = currentStation) {
    if (currentGame === 2) {
...
function buildSong(introSection, middleSection, outroSection) {
    if (currentGame === 2) {
...
function playSong(fSong, trackNumber, ad = false) {
...
function getRawSongs() {
    ...
    if (currentGame === 2) {
        ...
        let rawSongs = new Array(songPointers.length);
